...
emb = pd.read_csv(snakemake.input['embedding_path'], sep = '\t').set_index("ENSGID")

#subset to the experimental part of pops.
emb = emb.iloc[:, :40547]
dt = label_table.join(emb)

# ------- !!!!!!!!! -------
# DROPS GENES WHICH ARE PRESENT IN THE DATABASE BUT NOT IN THE EMBEDDING
dt = dt.dropna()
...
#         )
    ...
    # ------------ ElasticNet ------------
    reg = ElasticNetCV(l1_ratio=0.9, random_state=0)
